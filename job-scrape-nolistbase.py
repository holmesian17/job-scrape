...
if str(newPFL) != str(oldPFL):
    ...
    server.starttls()
    server.login('noreply.job.updates@gmail.com', 'zxcvbnm1029384756')
    server.sendmail('noreply.job.updates@gmail.com', 'holmesian17@gmail.com', 'Subject: PFL jobs page has changed\n' '\n Here is a link: https://libwww.freelibrary.org/jobs/JobList.cfm')
    server.quit()

res = requests.get('http://oppl.org/get-involved/jobs') # Oak Park Public Library
res.raise_for_status()
OPPLsoup = bs4.BeautifulSoup(res.text, "lxml")
newOPPL = OPPLsoup.select('#main')
file = open('/home/ian/libsitehtml/newOPPL', 'w')
file.write(str(newOPPL))
file.close()

newOPPL = open('/home/ian/libsitehtml/newOPPL')
newOPPL = newOPPL.read()

# opening and reading the old html doc as a string
oldOPPL = open('/home/ian/libsitehtml/oldOPPL')
oldOPPL = oldOPPL.read()

#this successfully changes the old document to the new website
if str(newOPPL) != str(oldOPPL):
    #saves the page as a file to the HD if there are changes
    file = open('/home/ian/libsitehtml/oldOPPL', 'w') 
    file.write(str(newOPPL))
    file.close()
    #emails me if there are changes
    server = smtplib.SMTP('smtp.gmail.com', 587)
    ...
    server.starttls()
    server.login('noreply.job.updates@gmail.com', 'zxcvbnm1029384756')
    server.sendmail('noreply.job.updates@gmail.com', 'holmesian17@gmail.com', 'Subject: OPPL jobs page has changed\n' '\n Here is a link: http://oppl.org/get-involved/jobs')
    server.quit()
